...
np_files = os.listdir('./npy_group2000w')

for npn in tqdm(np_files, total=len(np_files), leave=True):
    out1 = np.load(os.path.join('./npy_group2000w', npn))
...
for z1 in zu:
    ...
    wd = z1[0]
    wz = z1[1]
    savepath = f'2000w_{wd}_{wz}out'
    print(savepath, 'start')
    if not os.path.exists(savepath):
        os.mkdir(savepath)
    # PCA降维
    pca = PCA(n_components=wd)
    pca.fit(out_all[:20000000])
    ...
    print(numlenmax)
    np.save(os.path.join(savepath, f'inference0602_{wd}_{wz}.npy'), DataLink)
