...
if __name__ == '__main__':
    ...
    for i in range(epoch):
        ...
        # 训练集dataloader
        dataloader_train = DataLoader(dataset_train, batch_size=batch_size, shuffle=True, num_workers=30,
                                      drop_last=True)
        for idx, (data, data_) in tqdm(enumerate(dataloader_train), total=len(dataloader_train), leave=True):
            ...
            loss = torch.mean(-torch.log(
                torch.exp(torch.sum(y * y_, dim=1)) / torch.sum(torch.exp(torch.matmul(y, y_.transpose(0, 1))), dim=1)))
        ...
        if min_all_loss > all_loss / len(dataloader_train):
            ...
            torch.save(model, 'model_demo2.pth')
